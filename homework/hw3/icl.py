...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    """
    # YOUR CODE HERE
    order = list(range(len(support_inputs)))
    shuffle_order = np.random.permutation(order)
    shuffle_support_inputs = [support_inputs[idx] for idx in shuffle_order]
    shuffle_support_labels = [support_labels[idx] for idx in shuffle_order]
    prompt = ''
    if prompt_mode == 'qa':
        prompt = "".join([f"{q} In the {a}. " for q, a in zip(shuffle_support_inputs, shuffle_support_labels)])
        prompt += test_input + " In the"
    elif prompt_mode == 'none':
        prompt = "".join([f"{q} {a} " for q,a in zip(shuffle_support_inputs,shuffle_support_labels)])
        prompt += test_input
    elif prompt_mode == 'tldr':
        prompt = "".join([f"{q} TL;DR: {a} " for q,a in zip(shuffle_support_inputs,shuffle_support_labels)])
        prompt += test_input + " TL;DR:"
    elif prompt_mode == "custom":
        prompt = "".join([f"{q} Answer: {a}. " for q, a in zip(shuffle_support_inputs, shuffle_support_labels)])
        prompt += test_input + " Answer:"
    else:
        raise Exception("Unknown prompt_mode")
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    for i in range(max_tokens):
        ...
        logits = outputs.logits[:, -1, :] # (1,len)
        # print(f"logits shape {logits.shape}")
        predicted_id = logits.argmax(-1) # (1,1)
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            # proper device.
                            # YOUR CODE HERE
                            icl_prompts = get_icl_prompts(support_x, support_y, test_input, prompt_mode) # get prompt
                            encoded_prompts = tokenizer(icl_prompts, return_tensors='pt').to(DEVICE) # encode the prompt
                            sampled_tokens = do_sample(model, encoded_prompts.input_ids, stop_tokens, max_tokens) # using pretrained model to do sampling
                            decoded_prediction = tokenizer.decode(sampled_tokens) # decode the sampled tokens
                            # END YOUR CODE
...
def plot(models, dataset, ks, prompt_modes):
    ...
    plt.savefig(f"icl_{dataset}_{ks}_{prompt_modes}.png")
    plt.show()
