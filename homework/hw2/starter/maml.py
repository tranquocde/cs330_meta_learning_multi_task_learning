"""Implementation of model-agnostic meta-learning (optimize-based) for Omniglot."""
...
class MAML:
    ...
    def _inner_loop(self, images, labels, train): # design to update task-parameters
        ...
        """
        accuracies = []
        # initialize with meta_params (updated in outer-step)
        parameters = {
            k: torch.clone(v)
        ...
        for i in range(self._num_inner_steps + 1):
            ...
            # update parameters and inner learning rate 
            for (i, n) in enumerate(parameters.keys()):
                parameters[n] = parameters[n] - self._inner_lrs[n] * grad[i]
    ...
    def _outer_step(self, task_batch, train): # design to update meta-paramters ( in this case is the initialization for process of updating task-params.)
        """Computes the MAML loss and metrics on a batch of tasks.Then for updating meta-params in train()
        ...
        for task in task_batch:
            ...
            # initialize the adapted parameters
            adapted_parameters = {
                k: v.clone().detach().requires_grad_(True)
                for k, v in self._meta_parameters.items()
            }
            # run inner loop to get adapted params. and acc for support images
            adapted_parameters,acc_support = self._inner_loop(images_support,labels_support,train)
            # compute the loss on the query set
            logits = self._forward(images_query, adapted_parameters)
    ...
    def train(self, dataloader_train, dataloader_val, writer):
        ...
        for i_step, task_batch in enumerate(
            ...
            outer_loss.backward()
            # update self.meta_parameters and self.lr ( meta-parameters)
